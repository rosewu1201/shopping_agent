...
async function search(query) {
  ...
  const html = await fetchPage(url, 'walmart');
  ...
  if (nextDataScript) {
    try {
      ...
      for (const item of items.slice(0, 25)) {
        ...
        // Build the direct product URL
        let productUrl = '';
        if (item.canonicalUrl) {
          // canonicalUrl looks like /ip/Product-Name/123456?classType=REGULAR
      ...
      if (listings.length > 0) {
        log('SCRAPE', `Walmart (__NEXT_DATA__): found ${listings.length} listings for "${query}"`);
        return listings;
      }
    } catch (e) {
  ...
  $('[data-item-id]').each((i, el) => {
    ...
    // Walmart Barbie items are typically $5-$500
    if (price > 500) return;
  ...
  log('SCRAPE', `Walmart (DOM): found ${listings.length} listings for "${query}"`);
  return listings;
}
